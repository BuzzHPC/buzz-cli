...
func newCreateCmd(a app.App) *cobra.Command {
	var name, regionStr, nodeType, image string
	var gpuCount int
	...
	cmd := &cobra.Command{
		...
		Example: `  buzz notebook create --name my-nb --region ca-qc-2
  buzz nb create --name my-nb --region ca-qc-1 --node-type H100 --gpu-count 2
  buzz notebook create --name my-nb --region ca-qc-2 --image jupyter/scipy-notebook:latest`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			output.Info(fmt.Sprintf("Creating Notebook %q in %s/%s (region: %s)...", name, ref.Project, ref.Name, r))
	...
	cmd.Flags().StringVarP(&name, "name", "n", "", "Name of the Notebook (required)")
...
func newGetCmd(a app.App) *cobra.Command {
	return &cobra.Command{
		...
		Short:   "Get details of a Jupyter Notebook",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			output.Table([]string{"FIELD", "VALUE"}, [][]string{
				...
				{"URL", fmt.Sprintf("https://%s.notebook.buzzperformancecloud.com", args[0])},
			})
			return nil
		},
	}
...
func newDeleteCmd(a app.App) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ok, err := cmdutil.ConfirmDelete(force, "Notebook", args[0], nil)
			if err != nil {
				return err
			}
			if !ok {
